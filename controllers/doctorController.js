...
const applyfordoctor = async (req, res) => {
  try {
    const alreadyFound = await Doctor.findOne({ userId: req.body.userId });
    if (alreadyFound) {
      return res.status(400).send("Application already exists");
    }

    // Extract form details from the request body
    const { specialization, experience, fees, isDoctor } = req.body;
    ...
    // Create a new Doctor instance with extracted details
    const doctor = new Doctor({
      userId,
      specialization,
      experience,
      fees,
      isDoctor: true // Assuming this field should always be true when applying for a doctor
    });
